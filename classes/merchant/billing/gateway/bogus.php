 ...
 */
class Merchant_Billing_Gateway_Bogus extends Merchant_Billing_Gateway {
  ...
  public function authorize($money, Merchant_Billing_CreditCard $creditcard, $options = array()) {
    switch ($creditcard->number) {
      case '1':
        return new Merchant_Billing_Response(true, self::SUCCESS_MESSAGE, array('authorized_amount'=>$money), array('test'=>true, 'authorization' => self::AUTHORIZATION,'processor'=>'Bogus' ));
        break;
      case '2':
        return new Merchant_Billing_Response(false, self::FAILURE_MESSAGE, array('authorized_amount' => $money, 'error' => self::FAILURE_MESSAGE), array('test' => true,'processor'=>'Bogus'));
        break;
      default:
  ...
  public function purchase($money, Merchant_Billing_CreditCard $creditcard, $options = array()) {
    switch ($creditcard->number) {
      case '1':
        return new Merchant_Billing_Response(true, self::SUCCESS_MESSAGE, array('paid_amount'=>$money), array('test'=>true,'processor'=>'Bogus'));
        break;
      case '2':
        return new Merchant_Billing_Response(false, self::FAILURE_MESSAGE, array('paid_amount' => $money, 'error' => self::FAILURE_MESSAGE), array('test' => true,'processor'=>'Bogus'));
        break;
      default:
        throw new Exception(self::ERROR_MESSAGE);
        break;
  ...
  public function void($ident, $options = array()) {
    switch ($ident) {
      case '1':
        ...
        break;
      case '2':
        return new Merchant_Billing_Response(false, self::FAILURE_MESSAGE, array('authorization' => $ident, 'error' => self::FAILURE_MESSAGE), array('test' => true,'processor'=>'Bogus'));
        break;
      default:
        return new Merchant_Billing_Response(true, self::SUCCESS_MESSAGE, array('authorization'=>$ident), array('test'=>true,'processor'=>'Bogus'));
        break;
    }
  }
  ...
  public function store(Merchant_Billing_CreditCard $creditcard, $options = array()) {
    switch ($creditcard->number) {
      case '1':
        return new Merchant_Billing_Response(true, self::SUCCESS_MESSAGE, array('billingid' => '1'), array('test' => true, 'authorization' => self::AUTHORIZATION,'processor'=>'Bogus') );
        break;
      case '2':
        return new Merchant_Billing_Response(false, self::FAILURE_MESSAGE, array('billingid' => null, 'error' => self::FAILURE_MESSAGE ), array('test' => true,'processor'=>'Bogus') );
        break;
  ...
  public function unstore($identification, $options = array()) {
    switch ($identification) {
      case '1':
        return new Merchant_Billing_Response(true, self::SUCCESS_MESSAGE, array(), array('test' => true,'processor'=>'Bogus'));
        break;
      case '2':
        return new Merchant_Billing_Response(false, self::FAILURE_MESSAGE, array('error' => self::FAILURE_MESSAGE ), array('test' => true,'processor'=>'Bogus') );
        break;
